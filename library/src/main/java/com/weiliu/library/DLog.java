...
public class DLog {
    private static final String TAG = "xxxWEILIU_SQXBS";
    ...
    public static void longLog(int level, String str) {
        ...
        while (str.length() > index * maxLength) {
            ...
            if (str.length() < (index + 1) * maxLength) {
                WeiliuLog.log(level, str.substring(index * maxLength));
            } else {
                WeiliuLog.log(level, str.substring(index * maxLength, (index + 1) * maxLength));
            }
            index++;
        }
    }
