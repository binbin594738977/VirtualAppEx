...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    public CirclePageIndicator(@NonNull Context context, @NonNull AttributeSet attrs) {
        this(context, attrs, R.attr.vpiCirclePageIndicatorStyle);
    }

    public CirclePageIndicator(@NonNull Context context, @NonNull AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        if (mCentered) {
            longOffset += ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f) - (((count * 4 - 2) * mRadius) / 2.0f);
        }
        ...
        for (int iLoop = 0; iLoop < count; iLoop++) {
            ...
            if (Float.compare(pageFillRadius, mRadius) != 0) {
                canvas.drawCircle(dX, dY, mRadius - mPaintStroke.getStrokeWidth() / 2f, mPaintStroke);
            }
        }
    ...
    public boolean onTouchEvent(@NonNull MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (!mIsDragging) {
				...
				final float sixthWidth = width / 6f;	//SUPPRESS CHECKSTYLE
