    ...
    private static void assetType(@NonNull Type type, @NonNull String fieldOwner,
        ...
        if (!(type instanceof Class)) {
            if (type instanceof ParameterizedType) {    // 参数化类型，如HashMap<String, String>
                ...
                if ((type instanceof TypeVariable) && ((TypeVariable) type).getBounds().length == 1) {  // 有上界变量类型，如<T extends List>
                    assetType(((TypeVariable) type).getBounds()[0], fieldOwner, fieldName, 0, excludes, acceptTypeVariable);
                    return;
                }
        ...
        for (Field field : fields) {
            if (field.getAnnotation(JsonAccessType.class) != null) {
                continue;
            }
...
//        }
//
        if (Serializable.class.isAssignableFrom(cls) && cls.getName().startsWith("java.lang.")) {
            return true;
        }
